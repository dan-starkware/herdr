...
- Restoring a session now re-runs a pane's original launch command when there is no resumable agent conversation, instead of dropping to a plain shell, so agents come back as what was running. Manually-started agents (e.g. running `codex` in a shell) are relaunched from the detected agent recorded for the pane.
