...
# Command launched by the "new agent" flow inside the worktree.
agent_command = ["claude"]
# Git-ignored files symlinked from the source checkout into each new agent
# worktree, so agents inherit local setup git does not track. Defaults to
# [".cargo/config.toml"]; missing files are skipped and tracked files are never
# shadowed. Set to [] to disable, or add entries like ".env".
agent_symlink_paths = [".cargo/config.toml"]
```
