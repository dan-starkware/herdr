...
pub(super) fn render_agent_chooser_overlay(app: &AppState, frame: &mut Frame) {
    let popup = app.agent_chooser_popup_rect();
    if render_panel_shell(frame, popup, app.palette.accent, app.palette.panel_bg).is_none() {
        return;
    }
    render_search(app, frame, app.agent_chooser_search_rect());
    render_rows(app, frame, app.agent_chooser_body_rect());
    render_footer(app, frame, app.agent_chooser_footer_rect());
}
...
fn render_search(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let p = &app.palette;
    let query = app.agent_chooser.query.trim();
...
fn render_rows(app: &AppState, frame: &mut Frame, body: Rect) {
    ...
    let p = &app.palette;
    let indices = app.agent_chooser_filtered_indices();

    if indices.is_empty() {
        frame.render_widget(
            Paragraph::new(" no matching agents").style(Style::default().fg(p.overlay0)),
            Rect::new(body.x, body.y, body.width, 1),
        );
        return;
    }

    let start = app.agent_chooser.scroll.min(indices.len());
    let end = indices
        .len()
        .min(start.saturating_add(body.height as usize));
    for (visible_idx, &agent_idx) in indices[start..end].iter().enumerate() {
        let row_idx = start + visible_idx;
        let Some(agent) = app.agent_chooser.agents.get(agent_idx) else {
            continue;
        };
        let rect = Rect::new(body.x, body.y + visible_idx as u16, body.width, 1);
        let selected = row_idx == app.agent_chooser.selected;
        ...
        let label = truncate(agent, body.width.saturating_sub(2) as usize);
